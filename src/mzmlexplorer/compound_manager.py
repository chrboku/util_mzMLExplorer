...
class CompoundManager:
    ...
    def load_compounds(self, compounds_input):
        ...
        if isinstance(compounds_input, dict):
            ...
            if "Adducts" in compounds_input:
                ...
                if self.adducts_data.empty:
                    ...
                    new_adducts = new_adducts_data[
                        ~new_adducts_data["Adduct"].isin(existing_adducts)
                    ]
                    if not new_adducts.empty:
                        self.adducts_data = pd.concat(
                            [self.adducts_data, new_adducts], ignore_index=True
                        )
        ...
        for idx, row in compounds_data.iterrows():
            try:
                ...
                if (
                    ...
                    and str(row.get("ChemicalFormula")).strip()
                ):
                    ...
                    has_mass
                    and pd.notna(row.get("Mass"))
                    and str(row.get("Mass")).strip()
                ):
                    # Validate mass value
                    mass = float(row["Mass"])
                    if mass <= 0:
                ...
                print(
                    f"Warning: Invalid compound {row.get('Name', 'Unknown')}: {str(e)}"
                )
        ...
        if self.compounds_data.empty:
            ...
            self.compounds_data = pd.concat(
                [self.compounds_data, new_compounds_df], ignore_index=True
            )

        print(
            f"Added {len(valid_compounds)} new compounds. Total compounds: {len(self.compounds_data)}"
        )
    ...
    def _precalculate_mz_values(self):
            ...
            for adduct in adducts:
                try:
                    ...
                    self.compound_adduct_data[compound_name][adduct] = {
                        "mz": mz_value,
                        "polarity": polarity,
                        "display_name": self.get_adduct_display_name(
                            compound_name, adduct
                        ),
                    }
                except Exception as e:
                    print(
                        f"Warning: Could not calculate m/z for {compound_name} with {adduct}: {str(e)}"
                    )
                    self.compound_adduct_data[compound_name][adduct] = {
                        "mz": None,
                        "polarity": None,
        ...
        print(
            f"Pre-calculated m/z values for {len(self.compound_adduct_data)} compounds"
        )
    ...
    def get_compound_adducts(self, compound_name: str) -> List[str]:
        ...
        if compound is not None:
            ...
            if isinstance(adducts_str, str):
                return [
                    adduct.strip()
                    for adduct in adducts_str.split(",")
                    if adduct.strip()
                ]
        return []
    ...
    def calculate_compound_mz(self, compound_name: str, adduct: str) -> Optional[float]:
        ...
        try:
            ...
            if (
                adduct.startswith("[")
                and "]" in adduct
                and not any(letter in adduct for letter in "MHCNO")
            ):
            ...
            if compound_type == "formula":
                mz_value = calculate_mz_from_formula(
                    compound["ChemicalFormula"], adduct, self.adducts_data
                )
                return mz_value
    ...
    def search_compounds_by_mass(
        self, mass: float, tolerance: float = 0.01
    ) -> List[str]:
        """
        ...
        for _, compound in self.compounds_data.iterrows():
            ...
            if compound.get("compound_type") == "mass" and "Mass" in compound:
                ...
                compound.get("compound_type") == "formula"
                and "ChemicalFormula" in compound
            ):
                try:
                    compound_mass = calculate_molecular_mass(
    ...
    def get_compounds_summary(self) -> Dict:
        ...
        formula_count = len(
            self.compounds_data[self.compounds_data.get("compound_type") == "formula"]
        )
        mass_count = len(
            self.compounds_data[self.compounds_data.get("compound_type") == "mass"]
        )
    ...
    def validate_compound_data(self) -> List[str]:
        ...
        for idx, compound in self.compounds_data.iterrows():
            ...
            if rt_start >= rt_end:
                issues.append(
                    f"{compound_name}: RT_start_min must be less than RT_end_min"
                )

            if not (rt_start <= rt_center <= rt_end):
                issues.append(
                    f"{compound_name}: RT_min should be between RT_start_min and RT_end_min"
                )
            ...
            for adduct in adducts:
                ...
                if mz is None:
                    issues.append(
                        f"{compound_name}: Cannot calculate m/z for adduct {adduct}"
                    )
